...
@celery_app.task(name="tasks.resolve_predictions.run_hourly_resolution")
def run_hourly_resolution_task():
    asyncio.run(run_hourly_resolution())

async def run_hourly_resolution():
    """
    Syncs actual appointment wait times with their ML predictions.
    This allows us to calculate model drift over time.
    """
